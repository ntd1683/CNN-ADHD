...
#Tính điểm cho các cột khác
other_columns = {
    'emotions' : ['col6', 'col16', 'col17'],
    'autism' : ['col5', 'col6'],
    'stimulant_use' : ['col1', 'col2'],
    'hereditary_mental_illness' : ['col3', 'col4'],
}

# Tạo hàm để tính điểm cho mỗi đặc trưng
def calculate_score_other(row, feature):
    return sum(row[col] for col in other_columns[feature])

XOther = data.apply(lambda row: pd.Series({feature: calculate_score_other(row, feature) for feature in other_columns}), axis=1)
YOther = (XOther >= np.array([2,1,1,1])).sum(axis=1) >= 3
...
for column in other_columns:
    results_df[column] = XOther[column]
